...
- **Dépendances natives** : whisper.cpp compilé CMake/NDK avec bindings JNI (`token_timestamps=true`) ; ffmpeg-kit-full-gpl du fork ffmpegkit-maintained 8.1.7 (libass + libx264 + loudnorm, API drop-in `com.arthenica.ffmpegkit`) ; ML Kit Face Detection modèle bundled. **Alignement 16 KB page size obligatoire** (Android 16).
...
- **Compatibilité** : compileSdk 37 (amendement 2026-09-05 — imposé par les AAR androidx épinglés) / targetSdk 36 (Android 16) / minSdk 33 (Android 13), JDK toolchain 17, Gradle 9.4, AGP 9.1.0 — clause de fraîcheur : vérifier les versions à l'échafaudage et adopter la stable plus récente si existante
...
| compileSdk 37 (amendement E2) | Les AAR androidx épinglés exigent minCompileSdk=37 ; targetSdk/minSdk inchangés — impact build-time uniquement | — Committed |
| Stratégie de développement double cible (D-03/D-04) : émulateur x86_64 première cible rapide + téléphone physique ARM64 Android 13+ en adb USB pour la validation arm64 | Les `.so` arm64-only du fork ne sont jamais chargés en P1 — l'install émulateur x86_64 doit rester possible (prouvé en P1 : install Success) ; l'appareil physique valide la cible arm64 dès la Phase 2 (whisper.cpp JNI) | — Committed |
