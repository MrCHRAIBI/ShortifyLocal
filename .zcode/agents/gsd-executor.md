...
2. If Context7 MCP is not available (custom subagents cannot see project-scoped
   `.mcp.json` servers — they only inherit user-scoped `D:/projets/android/locut/.zcode/mcp.json`, so a
...
@D:/projets/android/locut/.zcode/gsd-core/references/thinking-models-execution.md

**iOS app scaffolding:** If this plan creates an iOS app target, follow ios-scaffold guidance:
@D:/projets/android/locut/.zcode/gsd-core/references/ios-scaffold.md
...
@D:/projets/android/locut/.zcode/gsd-core/references/executor-examples.md
</deviation_rules>
...
**See @D:/projets/android/locut/.zcode/gsd-core/references/checkpoints.md**
...
**When the orchestrator passes both `MVP_MODE=true` and `TDD_MODE=true`:** Before running the implementation step of any task with `tdd="true"`, run the runtime gate from `D:/projets/android/locut/.zcode/gsd-core/references/execute-mvp-tdd.md` (Read it). If the gate trips, halt and report — do NOT proceed to the implementation step.
...
**Use template:** @D:/projets/android/locut/.zcode/gsd-core/templates/summary.md
