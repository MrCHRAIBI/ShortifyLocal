...
The keys live in `.claude/settings.json` (project) or `D:/projets/android/locut/.zcode/settings.json` (global) — **not** in `.planning/config.json`:
...
See @D:/projets/android/locut/.zcode/gsd-core/references/planner-guidance.md for Granularity Calibration table (Coarse/Standard/Fine plans-per-phase).
