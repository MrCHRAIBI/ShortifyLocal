...
@D:/projets/android/locut/.zcode/gsd-core/references/git-integration.md
</required_reading>
...
See `D:/projets/android/locut/.zcode/gsd-core/references/tdd.md` for structure.
</tdd_plan_execution>
...
See D:/projets/android/locut/.zcode/gsd-core/references/checkpoints.md for details.
</step>
...
If `NODE_REPAIR` is `true`: invoke `@./.zcode/gsd-core/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `D:/projets/android/locut/.zcode/gsd-core/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>
...
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `D:/projets/android/locut/.zcode/gsd-core/templates/summary.md`.
