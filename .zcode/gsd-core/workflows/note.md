...
- **Global scope**: `D:/projets/android/locut/.zcode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/projets/android/locut/.zcode/notes/`)
...
2. Glob `D:/projets/android/locut/.zcode/notes/*.md` (if directory exists) — global notes
...
Global (D:/projets/android/locut/.zcode/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/projets/android/locut/.zcode/notes/` — works in any directory
