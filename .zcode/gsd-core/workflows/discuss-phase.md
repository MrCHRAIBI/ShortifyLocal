...
<required_reading>
@D:/projets/android/locut/.zcode/gsd-core/references/domain-probes.md
@D:/projets/android/locut/.zcode/gsd-core/references/gate-prompts.md
@D:/projets/android/locut/.zcode/gsd-core/references/universal-anti-patterns.md
</required_reading>
...
if [ -f "D:/projets/android/locut/.zcode/gsd-core/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
```bash
SPIKE_FINDINGS=$(ls ./.zcode/skills/spike-findings-*/SKILL.md 2>/dev/null | head -1 || true)
SKETCH_FINDINGS=$(ls ./.zcode/skills/sketch-findings-*/SKILL.md 2>/dev/null | head -1 || true)
...
Read `@D:/projets/android/locut/.zcode/gsd-core/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
...
```
Apply each entry in `activeHooks` per @D:/projets/android/locut/.zcode/gsd-core/references/loop-hook-dispatch.md. Empty list → continue to `analyze_phase`.
</step>
...
```
Apply each entry in `activeHooks` per @D:/projets/android/locut/.zcode/gsd-core/references/loop-hook-dispatch.md. Empty list → continue to `confirm_creation`.
</step>
