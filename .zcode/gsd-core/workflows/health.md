...
TASKS_DIR="D:/projets/android/locut/.zcode/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in D:/projets/android/locut/.zcode/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf D:/projets/android/locut/.zcode/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
